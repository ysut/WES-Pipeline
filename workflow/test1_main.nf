...
params.vep_options_ini = "${params.config_dir}/vep/options_copy.ini"
...
process EXTRACTCANONICAL {
    input:
    path vep_vcf

    output:
    path 'rare.vep.canon.vcf'

    script:
    """
    bcftools view \\
      --threads ${params.threads} \\
      --include 'INFO/CANONICAL="YES"' \\
      --output-type v \\
      --output rare.vep.canon.vcf \\
      ${vep_vcf}
    """
}
...
workflow {
    ...
    // vep_output_ch
    //     .map{ vepTxt, vepHtml -> tuple(vepTxt, vepHtml) }
    //     .set{ vep_txt_ch, vep_html_ch }

    // EXTRACTCANONICAL(vep_output_ch)
    //     .set{ vep_canonical_vcf_ch }

    // ANNOMAF(uncommon_vcf_ch)
    //     .set{ uncommon_maf_vcf_ch }

    // uncommon_maf_vcf_ch.view()

    // EXTRACTRARE(uncommon_maf_vcf_ch)
    //     .set{ rare_vcf_ch }
